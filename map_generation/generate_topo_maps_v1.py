...
Generates topographic base map + steepness overlay PNGs at 4 zoom levels
(matching Norgeskart's scale steps), centered on the same area as a cropped
DTM GeoTIFF. Because the bounding box is derived directly from the tif's own
...
OUTPUT_SIZE_PX = 2048  # width and height of every output PNG, in pixels

# The 4 zoom levels, matching Norgeskart's own scale steps.
# label is used in output filenames.
ZOOM_LEVELS = [
    {"label": "01_overview", "scale": 39810},
    {"label": "02_zoom",     "scale": 19905},
    {"label": "03_zoom",     "scale": 9952},
    {"label": "04_zoom",     "scale": 4976},
]
...
def main():
    ...
    for level in ZOOM_LEVELS:
        ...
        bbox = compute_bbox(center_x, center_y, scale, OUTPUT_SIZE_PX)
        print(f"Bounding box: {bbox}")

        print("Fetching base map...")
        base_img = fetch_kartverket_topo(bbox, OUTPUT_SIZE_PX)
        base_path = os.path.join(output_dir, f"{label}_base.png")
        base_img.save(base_path)
        print(f"Saved {base_path}")

        print("Fetching steepness overlay...")
        overlay_img = fetch_nve_steepness(bbox, OUTPUT_SIZE_PX)
